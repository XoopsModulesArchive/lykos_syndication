...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    if (!$result) {
        ...
        $temp .= '       <title>' . $xoopsConfig['sitename'] . " - Téléchargements</title>\n";
        ...
        while ($myrow = $xoopsDB->fetchArray($result)) {
            ...
            $temp .= '       <link>' . XOOPS_URL . '/modules/mydownloads/singlefile.php?cid=' . htmlspecialchars($myrow['lid'], ENT_QUOTES | ENT_HTML5) . '&amp;lid=' . htmlspecialchars($myrow['lid'], ENT_QUOTES | ENT_HTML5) . "</link>\n";

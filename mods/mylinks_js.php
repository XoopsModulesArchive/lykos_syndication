...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    $sql = 'SELECT lid, title FROM ' . $xoopsDB->prefix('mylinks_links') . ' WHERE status=1 ORDER BY date DESC';
    ...
    if (!$result) {
        ...
        $temp .= '<a href="' . XOOPS_URL . '/modules/mylinks">' . $xoopsConfig['sitename'] . " - Liens WEB</a><br></div>');\n";
        ...
        while ($myrow = $xoopsDB->fetchArray($result)) {
            ...
            $temp .= "document.write('* <span class=\"rss_body\"><A HREF=\"" . XOOPS_URL . '/modules/mylinks/index.php?lid=' . $myrow['lid'] . '">';

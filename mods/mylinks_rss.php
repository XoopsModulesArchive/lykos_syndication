...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    $sql = 'SELECT lid, cid, title FROM ' . $xoopsDB->prefix('mylinks_links') . ' WHERE status=1 ORDER BY date DESC';
    ...
    if (!$result) {
        ...
        $temp .= '       <title>' . $xoopsConfig['sitename'] . " - Liens WEB</title>\n";

        $temp .= '       <link>' . XOOPS_URL . "/mylinks</link>\n";
        ...
        while ($myrow = $xoopsDB->fetchArray($result)) {
            ...
            $temp .= '       <link>' . XOOPS_URL . '/modules/mylinks/singlelink.php?cid=' . htmlspecialchars($myrow['lid'], ENT_QUOTES | ENT_HTML5) . '&amp;lid=' . htmlspecialchars($myrow['lid'], ENT_QUOTES | ENT_HTML5) . "</link>\n";

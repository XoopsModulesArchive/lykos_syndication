...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    if (!$result) {
        ...
        $temp .= '<a href="' . $xoopsConfig['xoops_url'] . '">' . $xoopsConfig['sitename'] . " - Articles</a><br></div>');\n";

        while ($myrow = $xoopsDB->fetchArray($result)) {
            $myrow = str_replace('(', 'escape("(")', $myrow);

            $myrow = str_replace(')', 'escape(")")', $myrow);

            $myrow = str_replace("'", "escape(\"'\")", $myrow);

            $temp .= "document.write('* <span class=\"rss_body\"><A HREF=\"" . $xoopsConfig['xoops_url'] . '/modules/news/article.php?storyid=' . $myrow['storyid'] . '">';

            $temp .= $myrow['title'] . "</a></span><br>');\n";
        }

        $t = date('F j, Y, G:i', time() - 3600);       //For time diff
